...
2. **P2 클로저 잔여.** (정정 2026-07) ICG는 이미 탑에 인라인(`die_clk`, `glm_q4k_system.v:1307-1311`);
   ...
   **전 5개 die weight-input family에 대해 증명됨** — `make loopback`(aw) + `make loopback-fw`(fw) +
   `make loopback-rest`(rw/lw/gn; ALL 10, 커밋 bit-exact + **직접 per-beat 바이트 바인딩** — 출력-둔감한
   rw/gn은 die_rw_q/die_gn_val을 기대값과 매 beat 직접 비교해 바이트 소비를 강하게 증명). 잔여: top scan
   stitch(툴-삽입); 실 벤더 PHY IP. (payload ECC는 `weight_loader_q4k`+`kv_ecc_ring`으로 이미 됨.) → **C6·C7·C10**.
