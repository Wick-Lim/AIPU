`timescale 1ns/1ps
//============================================================================
// glm_q4k_loopback_fw_tb.v -- C8 LOOPBACK_FW=1 BIT-EXACTNESS proof for
//   glm_q4k_system, the EXACT mirror of glm_q4k_loopback_tb.v for the FFN
//   routed-expert (fw) weight CODE family instead of attention (aw).
//----------------------------------------------------------------------------
// WHAT IT PROVES
//   The perf/loopback TBs already prove glm_q4k_system committed next_tok ==
//   standalone glm_model_q4k fed by the SAME deterministic weight functions
//   (f_awq / f_rwq / f_fwq / ...).  This TB proves that equality with
//   LOOPBACK_FW=1 -- i.e. when the die's FFN-expert Q4_K CODE lanes (BOTH
//   buses: fw_q GATE/DOWN and fw_q_up UP) are physically routed OUT of the die
//   as ONE banked ddr5_xbar read (TAG_LBFW) and back IN through the fabric,
//   instead of being served same-cycle by the stub.  If the loopback round-trip
//   is faithful, the committed token stream is BIT-EXACT to the non-loopback
//   (== reference) stream: clock-gating a synchronous die is transparent, and
//   the fed-back lanes equal the stub lanes for the same key.
//
//   fw is the BANDWIDTH-DOMINANT family (~14 GB/token), so this closes the C8
//   gap for the family that matters most.  The aw loopback (LOOPBACK=1) is left
//   OFF here so the fw path is proven in ISOLATION, exactly as the aw TB proves
//   aw in isolation.  Both-off byte-identity is proven by `make resident-equiv`
//   / `make synth-glm`; either-on is proven by `make loopback` (aw) + this gate.
//
// HOW THE LOOPBACK-FW RESPONDER WORKS (the ONLY change vs the perf TB harness)
...
//   BOTH code buses into ONE beat (they fit: 8*TN = 32 bits <= DDR_DATA_W):
//       low  4*TN bits, lane t = f_fwq(layer, sel, shared, eidx, grp*TN+t, k)  (fw_q)
//       next 4*TN bits, lane t = f_fwq(layer, 3,   shared, eidx, grp*TN+t, k)  (fw_q_up)
//   -- EXACTLY the bytes the stub `fw_q`/`fw_q_up` present for that key at
//   LOOPBACK_FW=0 (perf/loopback TB) and the reference `r_fw_q`/`r_fw_q_up`
//   present.  (fw_q_up uses sel=3 fixed, mirroring the stub.)  Every OTHER read
//   keeps the perf TB's banked-read behaviour (gen_beat).  Only fw-loopback reads
//   carry the 8'hB6 marker in addr[31:24] (TAG_LOAD/SLOT/HOT/EFILL/LBAW all use
//   0 or 8'hA5), so the decode is unambiguous.
//
// SOUNDNESS (-DLBFWINJECT): the loopback-returned fw lane 0 is XOR-corrupted
//   (bit 0 flipped on EVERY loopback beat).  If the die actually CONSUMES the
//   fed-back lanes, the corrupted FFN weights move the committed argmax -> the
...
//   "ALL %0d TESTS PASSED ...".  Mirrors test/glm_q4k_loopback_tb.v idioms.
//============================================================================
module glm_q4k_loopback_fw_tb;

    // ---- small-but-faithful slice (the perf/loopback TB geometry, PE_N=2) ----
    ...
    // aw LOOPBACK held OFF: this gate proves the fw path in isolation (the fw
    // loopback branch owns the fw-enable term of die_clk).  RESIDENT/EXPERT_STALL
    // OFF -- neither is needed to exercise the fw loopback path.
    parameter integer LOOPBACK_CFG     = 0;
    ...
    // ================= slice params (mirror the loopback TB) =================
    ...
    //========================================================================
    // DUT (the Q4_K system) host I/O -- LOOPBACK_FW=1
    //========================================================================
    reg                       start;
    ...
    //   NOTE: at LOOPBACK_FW=1 the die's FFN CODE lanes come from the xbar
    //   round-trip, NOT from these `fw_q`/`fw_q_up` inputs (die_fw_q=lbfw_q_q,
    //   die_fw_q_up=lbfw_qup_q).  They are still driven (harmlessly) so the port
    //   matches the perf/loopback TB exactly; the fw d/dmin/scales stubs ARE still
    //   consumed same-cycle by the die.
    integer t, ft, re, sb;
    ...
    wire _unused = &{1'b0, busy, em_req, aw_req, fw_req, rw_req, gn_req, fn_req,
                     ...
                     kv_row_valid, fw_q, fw_q_up, xbar_resp_data, loader_w_q,
    ...
    // ================= DDR5 PER-CHANNEL MEMORY MODEL + C8 LOOPBACK-FW RESPONDER ==
    ...
    //   LOOPBACK-FW read -- addr[24 +: 8]==8'hB6 -- the beat's low 4*TN bits are
    //   the packed f_fwq GATE/DOWN lanes and the next 4*TN bits the f_fwq UP lanes,
    //   both decoded from the address (the die's fw code buses).
    localparam [7:0] TAG_LBFW_MARK = 8'hB6;     // addr[24 +: 8] on an fw-loopback read
    ...
    integer ii, cc;
    integer lbfw_req_seen, lbfw_resp_seen;    // fw-loopback-path activity counters
    ...
    // C8 LOOPBACK-FW beat: decode {layer,sel,shared,eidx,grp,k} from the marked
    // address and pack BOTH code buses (low 4*TN = GATE/DOWN f_fwq(...,sel,...),
    // next 4*TN = UP f_fwq(...,3,...)) into one beat.  The high bits stay X-clean
    // (gen_beat base) since the xbar-resp X-monitor checks the WHOLE beat; the die
    // only consumes bits [8*TN-1:0].
    function automatic [DDR_DATA_W-1:0] gen_lbfw_beat;
        input [DDR_TAG_W-1:0] tg; input [DDR_ADDR_W-1:0] ad;
        integer ln; integer ly, sel, shr, eidx, grp, kk;
...
`ifdef LBFWINJECT
        // SOUNDNESS INJECTION: corrupt the fed-back fw lane 0 (flip bit 0) on EVERY
...
`endif
    ...
    always @* begin
        ...
        for (cc=0; cc<DDR_NCH; cc=cc+1) begin
            ...
            if (presV[cc]) begin
                ...
                // C8 LOOPBACK-FW decode when the address carries the TAG_LBFW marker;
                // every other read keeps the normal banked beat.
                if (infad[presIdx[cc]][24 +: 8] == TAG_LBFW_MARK)
                    mem_resp_data[cc*DDR_DATA_W +: DDR_DATA_W] =
                        gen_lbfw_beat(inftg[presIdx[cc]], infad[presIdx[cc]]);
                else
                    mem_resp_data[cc*DDR_DATA_W +: DDR_DATA_W] =
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            lbfw_req_seen <= 0; lbfw_resp_seen <= 0;
        end else begin
            ...
            for (cc=0; cc<DDR_NCH; cc=cc+1)
                if (presV[cc] && mem_resp_ready[cc]) begin
                    ...
                    if (infad[presIdx[cc]][24 +: 8] == TAG_LBFW_MARK)
                        lbfw_resp_seen <= lbfw_resp_seen + 1;   // an fw-loopback beat drained
                end
            ...
            for (cc=0; cc<DDR_NCH; cc=cc+1) begin
                if (mem_req_valid[cc] && mem_req_ready[cc] && got_free) begin
                    ...
                    if (mem_req_addr[cc*DDR_ADDR_W+24 +: 8] == TAG_LBFW_MARK)
                        lbfw_req_seen <= lbfw_req_seen + 1;     // an fw-loopback read captured
                end
            end
        end
    end
    ...
    task run_token; input [TOKW-1:0] tk; input [POSW-1:0] ps; input integer SL;
        ...
        // (a) BINDING: LOOPBACK_FW=1 system committed token == standalone reference
        if (next_tok !== r_tok_lat) begin
            $display("FAIL[%0s]: BINDING next_tok %0d != standalone ref %0d (LOOPBACK_FW=1 diverged)",
        ...
        $display("PASS[%0s] tok=%0d(==ref %0d) | xbar req/resp=%0d/%0d | fw loopback reads/resps=%0d/%0d",
                 label, next_tok, r_tok_lat, xbar_req_count, xbar_resp_count,
                 lbfw_req_seen, lbfw_resp_seen);
    end endtask
    ...
    initial begin
        ...
        // ---- global soundness gate: the fw loopback path MUST have round-tripped ----
        //   (proves the bindings above actually flowed BOTH fw code buses through the
        //   real ddr5_xbar, not a bypass).
        test_count = test_count + 1;
        if (lbfw_req_seen == 0 || lbfw_resp_seen == 0) begin
            $display("FAIL: C8 fw-loopback path never exercised (lbfw_req=%0d lbfw_resp=%0d) -- the proof is vacuous",
                     lbfw_req_seen, lbfw_resp_seen);
            errors=errors+1;
        end else
            $display("PASS[loopback-fw-exercised] lbfw_reads=%0d lbfw_resps=%0d routed through ddr5_xbar",
                     lbfw_req_seen, lbfw_resp_seen);

        if (errors!=0) begin
            $display("FAILED: %0d error(s) across %0d tests (LOOPBACK_FW=1 not bit-exact to reference)",
                     errors, test_count);
            $fatal;
        end
        $display("ALL %0d TESTS PASSED  (glm_q4k_system LOOPBACK_FW=1: die FFN-expert code lanes fw_q+fw_q_up routed OUT and back IN through ddr5_xbar, committed stream BIT-EXACT to standalone glm_model_q4k reference, 4-token decode)",
                 test_count);
        $finish;
    end
endmodule
