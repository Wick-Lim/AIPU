`timescale 1ns/1ps
//============================================================================
// glm_q4k_self_kv_roundtrip_tb.v
//   KV latent WRITE-BACK round-trip proof (KV_WRITEBACK_DESIGN.md step 2, L=1).
//----------------------------------------------------------------------------
// WHAT IS PROVEN (self-checking; $fatal on any mismatch):
//
//   The DUT is glm_q4k_system with SELF_KV=1: the die's committed latent
//   (kv_lat_row / kv_lat_valid, exposed by step 1) is APPENDED to kv_cache_pager,
//   and the pager's row_out / row_valid feed the model's kc_ckv / kc_krope /
//   kc_valid -- so each decoded token READS the prior tokens' KV FROM THE PAGER
//   and APPENDS its own.  A multi-token decode therefore has the die attend to
//   KV it produced (positions 0..s_len-1), across a PERSISTENT pager.
//
//   The REFERENCE is an INDEPENDENT standalone glm_model_q4k fed the SAME
//   deterministic weights, whose KV is delivered by a TB-managed SHADOW MEMORY:
//   the TB captures the reference's OWN committed latent (r_kv_lat_row on
//   r_kv_lat_valid) into shadow[pos], and answers every reference gather from
//   shadow[kc_idx].  So the reference's KV for token t is exactly the latents it
//   computed for tokens 0..t-1 -- the same append/gather round-trip, transported
//   by an independent path (a TB array + TB unpack) instead of the pager.
//
//   BINDING: the system's committed next_tok === the reference's committed token,
//   EVERY token.  Both models compute identical latents (same weights / prompt /
//   prior KV), so they agree IFF the pager transport (pack -> append -> ring ->
//   gather -> row_out -> unpack) delivers bit-identical KV to what the TB shadow
//   delivers.  A mis-packed field or a mis-indexed append makes the DUT attend
//   corrupted KV and DIVERGE -- see the INJECTION note below.
//
//   Also checked, per token: the DUT pager appended EXACTLY ONE row (append_count
//   grew by 1 -- the write-back path, not the host stub) and the committed token
//   is X/Z-clean and === the system's internal argmax.
//
// INJECTION SOUNDNESS (run separately, tools/ or by hand -- see the Makefile
//   gate `self-kv-roundtrip`): swapping the row halves in glm_q4k_system's
//   SELF_KV unpack (kc_ckv <-> kc_krope) -- a real "swapped field" bug that only
//   the DUT sees (the reference uses the TB unpack) -- makes the DUT attend
//   corrupted KV and this TB FAILS the binding check.  A round-trip that could
//   not tell a mis-packed KV from a correct one would be worthless.
//
// STYLE: sync active-high reset; self-checking ($fatal on mismatch); final
//   banner "ALL %0d TESTS PASSED".
//============================================================================
module glm_q4k_self_kv_roundtrip_tb;

    // ---- L=1 slice (the perf-TB proven geometry; L=1, dense single layer) ----
    parameter integer MODEL_DIM = 16;
    parameter integer L         = 1;    // KV_WRITEBACK step 2 scope: L=1
    parameter integer N_DENSE   = 1;    // layer 0 dense (db_layer 0 < N_DENSE)
    ...
    localparam integer KV_RESIDENT = 16;   // >= S_MAX; N_TOK positions all resident
    ...
    //   Same deterministic weights; KV delivered by a TB SHADOW that captures the
    //   reference's OWN committed latents (r_kv_lat_row) and answers gathers from it.
    reg                       r_start;
    wire                      r_busy, r_done;
    ...
    // ---- REFERENCE KV SHADOW: capture the ref's OWN committed latent, serve gathers ----
    //   shadow[pos] = the latent the reference committed at position pos (r_kv_lat_row).
    //   ref_wr advances once per token (one commit per L=1 run).  The gather answer
    //   UNPACKS the row the same way glm_q4k_system does (c_kv LOW, k_rope HIGH) --
    //   an independent TB implementation of the pack convention.
    reg [ROW_BITS-1:0] shadow [0:KV_CTX-1];
    reg [KVPOSW-1:0]   ref_wr;
    integer shi;
    initial for (shi=0; shi<KV_CTX; shi=shi+1) shadow[shi] = {ROW_BITS{1'b0}};
    always @(posedge clk) begin
        if (rst) ref_wr <= {KVPOSW{1'b0}};
        else if (r_kv_lat_valid) begin
            shadow[ref_wr] <= r_kv_lat_row;
            ref_wr <= ref_wr + 1'b1;
        end
    end
    always @* begin
        r_kc_ckv   = shadow[r_kc_idx][0          +: KV_LORA*16];
        r_kc_krope = shadow[r_kc_idx][KV_LORA*16 +: ROPE*16];
    end
    always @(posedge clk) begin
    ...
    //   The empty-KV first token (s_len=0) makes the model issue gathers the empty
    //   pager cannot serve from the ring (cold); answer them (FLASH_LAT-cycle) so the
    //   die never hangs.  Their data is MASKED (u_cnt=0 -> zero attention) so the
    //   value is don't-care; both DUT and shadow-ref see zero context for that token.
    wire [KVPOSW-1:0] flash_row_idx_w = flash_row_idx;
    reg [31:0]         fl_timer; reg fl_active; reg prev_freq;
    // Return ZERO for cold rows -- IDENTICAL to the reference shadow's init-0 value.
    //   Cold rows are only ever pulled by the empty-KV first token (s_len=0), whose
    //   gathers are MASKED (u_cnt=0 -> zero attention); returning the SAME zero the
    //   reference sees keeps DUT and reference bit-identical for that token regardless
    //   of how the all-masked softmax treats the (unused) value.  Real (non-cold)
    //   gathers for tokens 1..N-1 come from the resident ring, never Flash.
    ...
    // ================= continuous X monitor on the committed token ==============
    // (no per-cycle DUT drive; pure observation)
    ...
    task run_token; input [TOKW-1:0] tk; input [POSW-1:0] ps; input integer SL;
        ...
        // (1) BINDING: SELF_KV=1 pager round-trip token == independent shadow ref
        if (next_tok !== r_tok_lat) begin
            $display("FAIL[%0s]: BINDING next_tok=%0d != shadow-ref token=%0d (SELF_KV round-trip diverged)",
                     label, next_tok, r_tok_lat);
            errors = errors + 1;
        end
        // (1b) BIT-EXACT LOGIT binding: the full pre-argmax logit vector must match
        //   the independent reference.  Far more sensitive than argmax alone -- any
        //   KV-transport corruption (mis-packed field, mis-indexed append) perturbs
        //   the attention context and shifts logits even when the argmax happens not
        //   to move.  This is what makes the injection test bite.
        if (logits !== r_logits) begin
            $display("FAIL[%0s]: BINDING logits != shadow-ref logits (SELF_KV round-trip KV corrupted)", label);
            errors = errors + 1;
        end
        ...
        if (next_tok !== argmax_o) begin
            ...
            errors=errors+1; end
        // (3) the WRITE-BACK path appended EXACTLY ONE row this token (not the stub)
        if (kv_append_count !== abefore + 1'b1) begin
            $display("FAIL[%0s]: append_count=%0d expected %0d (should grow by 1/token)",
                     label, kv_append_count, abefore + 1);
            errors=errors+1; end
        // (4) the pager actually served a gather row (except the first, empty-KV token)
    ...
    initial begin
        #8000000; $display("FAIL: global timeout"); $fatal(1, "timeout");
    end
    ...
    initial begin
        ...
        // ---- self-attending decode: pos=t, s_len=t; token t reads L0..L_{t-1} ----
        //   token 0 has EMPTY KV (s_len=0); each later token READS the pager rows
        //   that the earlier tokens WROTE, and appends its own.
        run_token(4'd5,     20'd0, 0, "tok0 s0 (empty KV)");
        run_token(next_tok, 20'd1, 1, "tok1 s1 (reads L0)");
        run_token(next_tok, 20'd2, 2, "tok2 s2 (reads L0,L1)");
        run_token(next_tok, 20'd3, 3, "tok3 s3 (reads L0,L1,L2)");

        if (errors != 0) begin
            $display("FAILED: %0d error(s) across %0d tests", errors, test_count);
            $fatal(1, "glm_q4k_self_kv_roundtrip_tb: SELF_KV=1 round-trip mismatch");
        end
        $display("ALL %0d TESTS PASSED  (glm_q4k_system SELF_KV=1 KV write-back round-trip == independent shadow-fed glm_model_q4k, %0d-token self-attending decode; die attends KV it produced)",
