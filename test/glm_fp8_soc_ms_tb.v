...
module glm_fp8_soc_ms_tb;
    ...
    localparam integer S_MAX      = 4;            // >= B*TOPK_ATTN (union depth)
    ...
    integer rc;
    task run_ref_row; input integer b; input integer tokb; input integer sqid; integer wd; begin
        g_seq = sqid[SEQW-1:0];               // reference DUT attends this sequence's KV
        token_id_s = tokb[TOKW-1:0];
        @(negedge clk); start_s = 1'b1; @(negedge clk); start_s = 1'b0;
        wd = 0; rc = 0;
        while (!done_s && wd < 400000) begin
            @(negedge clk);
            if (aw_req_s) rc = rc + 1;
            wd = wd + 1;
        end
        if (!done_s) begin $display("FAIL: ref row=%0d TIMEOUT", b); errors=errors+1; end
        @(negedge clk);
    ...
    task run_batched_and_check; input integer tokA; input integer tokB; begin
        ...
        for (sqi=0; sqi<B; sqi=sqi+1)                      // per-seq prompt length = s_len
            slen_w[(IDXW+1)*sqi +: (IDXW+1)] = s_len;
        @(negedge clk); start_w = 1'b1; @(negedge clk); start_w = 1'b0;
        wd2 = 0; rc2 = 0;
        while (!done_w && wd2 < 400000) begin
            @(negedge clk);
        ...
        // multi-seq: the query-side attention weights are SHARED across the B
        //   sequences, so the batched aw_req beats are STRICTLY FEWER than two
        //   independent single runs (2*aw_single).  (W_uk/W_uv scale with distinct
        //   keys, so it is not equal to a single run as in the shared-seq PE_M TB.)
        if (rc2 >= 2*aw_single) begin
            $display("FAIL: batched aw_req=%0d not < 2x single %0d (weights NOT shared across seqs)", rc2, 2*aw_single);
            fail_this=1;
        end
        if (fail_this) errors = errors + 1;
        else $display("ok  multi-seq {tokA=%0d/seq0, tokB=%0d/seq1}: row argmax {%0d,%0d} == per-seq ref; logits bit-exact; aw_req batched=%0d < 2x single=%0d (query-side shared)",
                      tokA, tokB, ref_argmax[0], ref_argmax[1], rc2, 2*aw_single);
    end endtask

    task do_case; input integer tokA; input integer tokB; begin
        run_ref_row(0, tokA, 0);   // token A against sequence 0's KV window
        run_ref_row(1, tokB, 1);   // token B against sequence 1's KV window
    ...
    initial begin
        ...
        //   each row attending its OWN sequence's KV window (row r -> seq r).  The SoC
        //   decodes each sequence at position = its s_len, so the PE_M=1 REFERENCE must
