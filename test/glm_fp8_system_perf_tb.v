...
//============================================================================
//============================================================================
// glm_fp8_system_perf_tb.v -- CYCLE-ACCURATE THROUGHPUT harness (derived from
//   glm_fp8_system_tb.v).  Same stimulus + same functional binding check
//   (committed token == independent standalone glm_model_fp8 reference), plus a
//   cycle counter that measures cycles-per-token (start->tok_valid) and emits a
//   machine-readable PERF line for the tools/perf_sweep.sh sweep.
//
//   The three memory knobs are exposed as top-level parameters (overridable at
//   compile via `iverilog -P glm_fp8_system_perf_tb.<PARAM>=<v>`):
//       FLASH_LAT_CFG   -- Flash fetch latency (drives BOTH the DUT .FLASH_LAT
//                          AND the TB Flash-PHY responder timer -> stall grows)
//       DDR_NCH_CFG     -- DDR5 channels (power of two)
//       CACHE_SLOTS_CFG -- expert-cache slots
//   These feed the existing FLASH_LAT / DDR_NCH / CACHE_SLOTS localparams, which
//   are ALREADY wired into both the DUT instantiation and the Flash stub, so the
//   knobs are live end-to-end.
//============================================================================
module glm_fp8_system_perf_tb;
    ...
    // ---- memory system ---- (driven by the overridable sweep knobs) ----
    ...
    // ---- DDR5 fabric + loader ----
    localparam integer DDR_NCH     = DDR_NCH_CFG;   // sweep knob (power of two)
    ...
    //   Free-running clock counter; each run_token records the #posedge clk from
    //   the cycle `start` is asserted to the cycle `tok_valid` fires (that token's
    //   cyc_per_tok).  Token 0 is the COLD-cache token; tokens 1..2 are warmer.
    reg  [63:0] perf_cyc;
    ...
    task run_token; input [TOKW-1:0] tk; input [POSW-1:0] ps; input integer SL;
        ...
        $display("PASS[%0s] tok=%0d(==ref %0d) hit/miss=%0d/%0d flash+=%0d kv=%0d rv+=%0d | xbar req/resp+=%0d/%0d loader done/beat+=%0d/%0d",
