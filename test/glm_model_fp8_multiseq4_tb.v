...
`include "glm_fp.vh"
//============================================================================
// glm_model_fp8_multiseq_tb.v -- MULTI-SEQUENCE (PER_ROW_SEQ=1) full-model batch
//                           for the FULL GLM-5.2-FP8 forward (glm_model_fp8.v, A2)
//----------------------------------------------------------------------------
...
//============================================================================
module glm_model_fp8_multiseq4_tb;
    ...
    localparam integer S_MAX      = 8;            // >= B*TOPK_ATTN (union depth, B=4)
    ...
    localparam integer B          = 4;            // FOUR sequences batched (scale-up)
    localparam integer NSEQ       = 4;            // distinct sequences (row r -> seq r)
    ...
    wire                      kc_req_w; wire [IDXW-1:0] kc_idx_w; wire [SEQW-1:0] kc_seq_w; reg [KV_LORA*16-1:0] kc_ckv_w; reg [ROPE*16-1:0] kc_krope_w; reg kc_valid_w;
    ...
    glm_model_fp8 #(
        ...
        .PER_ROW_SEQ(1), .SWIN(MS_SWIN)
    ) dut_w (
        .clk(clk), .rst(rst), .start(start_w), .busy(busy_w), .done(done_w),
        .token_id(token_id_w), .pos(pos), .s_len(s_len), .seq_vec(seq_w),
        ...
        .kc_req(kc_req_w), .kc_idx(kc_idx_w), .kc_seq(kc_seq_w), .kc_ckv(kc_ckv_w), .kc_krope(kc_krope_w), .kc_valid(kc_valid_w),
    ...
    integer sqi;
    reg [TOKW-1:0]     cse_tok [0:B-1];                    // per-row token (filled by caller)
    task run_batched_and_check; begin
        test_count = test_count + 1;
        for (sqi=0; sqi<B; sqi=sqi+1) begin                // row r = token r, sequence r
            token_id_w[TOKW*sqi +: TOKW] = cse_tok[sqi];
            seq_w[SEQW*sqi +: SEQW]      = sqi[SEQW-1:0];
        end
        ...
        //   sequences, so the batched aw_req beats are STRICTLY FEWER than B
        //   independent single runs (B*aw_single).  (W_uk/W_uv scale with distinct
        //   keys, so it is not equal to a single run as in the shared-seq PE_M TB.)
        if (rc2 >= B*aw_single) begin
            $display("FAIL: batched aw_req=%0d not < Bx single %0d (weights NOT shared across seqs)", rc2, B*aw_single);
            fail_this=1;
        end
        if (fail_this) errors = errors + 1;
        else $display("ok  multi-seq B=%0d: all %0d rows argmax == per-seq ref; logits bit-exact; aw_req batched=%0d < Bx single=%0d (query-side shared)",
                      B, B, rc2, B*aw_single);
    end endtask

    integer dci;
    task run_all; begin                          // cse_tok[0..B-1] must be filled first
        for (dci = 0; dci < B; dci = dci + 1)
            run_ref_row(dci, cse_tok[dci], dci);   // token r against sequence r's KV window
        run_batched_and_check;
    end endtask
    ...
    initial begin
        ...
        // SCALE-UP: B=4 DIFFERENT sequences (users) batched in ONE forward, each row
        //   attending its OWN sequence's KV window (row r -> seq r).  4 distinct tokens
        //   -> divergent per-row MoE routing.  DENSE (S<=TOPK) then SPARSE (S>TOPK).
        pos = 0;  s_len = 2;     build_stimulus(500,  0);
        cse_tok[0]=3; cse_tok[1]=11; cse_tok[2]=5; cse_tok[3]=9;  run_all;
        pos = 42; s_len = S_MAX; build_stimulus(90000, 1);
        cse_tok[0]=5; cse_tok[1]=9; cse_tok[2]=2; cse_tok[3]=13; run_all;
