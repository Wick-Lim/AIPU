...
module glm_model_fp8_2x_tb;
    ...
    // ============= INTERMEDIATE (2x) config -- F0 cross-check =============
    // 2x on MODEL_DIM / L / N_EXPERT (and INTER); also forces multi-K-block FP8
    // scales: A_NB=2 (attn), R_NB=2 (router), FF_NB_D=4 (dense).  VOCAB & S_MAX
    // reduced to keep the sim tractable for a single vector.
    localparam integer MODEL_DIM  = 256;
    localparam integer L          = 6;
    localparam integer N_DENSE    = 3;
    localparam integer VOCAB      = 128;
    ...
    localparam integer INTER_MOE  = 128;
    localparam integer INTER_DENSE= 384;
    ...
    always @(fw_grp or fw_k or fw_sel or fw_shared or fw_eidx or db_layer or start) begin
        ...
        for (ft=0;ft<TN;ft=ft+1) begin
            if (dmode==1'b0) begin
                // F0 2x: generalise the committed (hardcoded-2-block) dense scale
                // responder to ALL FF_NB_D K-block lanes.  down: ScDd[kb] per
                // K-block; gate/up: ScDg/ScDu[obd] uniform over K-blocks.  At the
                // committed slice (FF_NB_D=2) this reduces to the original.
                if (fw_sel==2'd2) begin
                    for (bj=0;bj<FF_NB_D;bj=bj+1)
                        fw_scale_g[16*(bj*TN+ft)+:16]=ScDd[db_layer][bj];
                end else begin
                    for (bj=0;bj<FF_NB_D;bj=bj+1) begin
                        fw_scale_g[16*(bj*TN+ft)+:16]=ScDg[db_layer][obd];
                        fw_scale_u[16*(bj*TN+ft)+:16]=ScDu[db_layer][obd];
                    end
                end
            end else begin
    ...
    integer fdump, dmp_o;   // F0 cross-check: dump golden + DUT logits per test
    integer fstage, stg_call, stg_j;  // F0: per-layer golden stage dump
    ...
    task compute_golden; begin
        ...
        for (GLY=0;GLY<L;GLY=GLY+1) begin
            layer_golden(GLY, (GLY<N_DENSE)?0:1);
            // F0 stage dump: per (call,layer,j): nrm1 attn h nrm2 ffn x (bf16 codes)
            $fwrite(fstage, "STAGE call %0d layer %0d\n", stg_call, GLY);
            for (stg_j=0; stg_j<MODEL_DIM; stg_j=stg_j+1)
                $fwrite(fstage, "%0d %04x %04x %04x %04x %04x %04x\n", stg_j,
                        g_nrm1[stg_j], real2b(g_attn[stg_j]), g_h[stg_j],
                        g_nrm2[stg_j], real2b(g_ffn[stg_j]), g_x[stg_j]);
        end
        stg_call = stg_call + 1;
    ...
    task check_out; input [256*8-1:0] label; begin
        ...
        // ---- F0 cross-check dump: per-logit golden(bf16 code) + DUT(bf16 code) ----
        $fwrite(fdump, "TEST %0d tok %0d pos %0d S %0d band %0d g_argmax %0d dut_argmax %0d\n",
                test_count, tok, tpos, Sg, tband, g_argmax, argmax);
        for (dmp_o=0; dmp_o<VOCAB; dmp_o=dmp_o+1)
            $fwrite(fdump, "%0d %04x %04x\n", dmp_o, real2b(g_logit[dmp_o]),
                    logits[16*dmp_o+:16]);
    end endtask
    ...
    initial begin
        ...
        fdump=$fopen("build_f0/tb_dump2.txt","w");
        fstage=$fopen("build_f0/tb_stage2.txt","w"); stg_call=0;
        ...
        tband=0; tok=7;   tpos=5;   Sg=S_MAX; build_stimulus(500,  tband); compute_golden(); run_dut(); check_out("2x tok7 pos5 Smax");
