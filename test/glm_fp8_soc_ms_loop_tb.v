...
`include "glm_fp.vh"
//============================================================================
// glm_fp8_soc_ms_tb.v -- MULTI-SEQUENCE batched SoC top (glm_fp8_soc_ms, A2 stage3b):
//   the full deployable batched decode -- host FSM prefills B sequences into a REAL
//   multi-seq kv_cache_pager, runs ONE PE_M=B forward, commits B next tokens.  Each
//   row's committed token is BIT-EXACT to an independent PE_M=1 model on that token
//   against that sequence's own KV window; the query-side weight stream is shared.
//----------------------------------------------------------------------------
// VERIFY STANCE  (RTL-vs-RTL)
//   glm_model_fp8_pem_tb pins the PE_M>1 widening for ONE shared sequence.  This
//   TB pins the MULTI-SEQUENCE contract (A2): B DIFFERENT sequences batched:
//
//     "row r of a PE_M=B / PER_ROW_SEQ=1 forward equals a standalone PE_M=1
//      forward on token r AGAINST ROW r's OWN KV WINDOW (sequence r), routed
//      end-to-end by kc_seq (model->decoder->mla->cache), while the query-side
//      weight stream is SHARED across the B sequences."
//
//   Two PE_M=1 references (token A vs seq 0, token B vs seq 1) are compared to
//   the row0/row1 of a PE_M=2 / PER_ROW_SEQ=1 batched forward whose cache
//   responder is KEYED BY kc_seq -> CKV_ms[seq][layer][idx].  A wrong kc_seq
//   would fetch the wrong window and the per-row argmax/logits would mismatch.
//
//   CHECKS:
//     * argmax[row r] === reference argmax for (token r, seq r)  (EXACT, X-aware)
//     * logits[row r] === reference logits for (token r, seq r)  (BIT-EXACT bf16)
//     * aw_req pulses(batched) <  2x a single run  (query-side weight stream shared)
//   Prints "ALL <N> TESTS PASSED"; $fatal on any mismatch / X / timeout.
//   Requires S_MAX >= B*TOPK_ATTN and the batched DUT SWIN = B*TOPK_ATTN.
//============================================================================
module glm_fp8_soc_ms_loop_tb;
    ...
    localparam integer S_MAX      = 8;            // >= prompt_len + N_STEPS (decode positions)
    ...
    localparam integer N_STEPS    = 2;            // decode steps per start (multi-token loop)
    localparam integer NSEQ       = 2;            // distinct sequences (row r -> seq r)
    localparam integer SEQW       = (B<=1)?1:$clog2(B);
    localparam integer MS_SWIN    = B*TOPK_ATTN;  // batched DUT attention scratch depth
    ...
    reg [15:0] CKV [0:NSEQ-1][0:L-1][0:S_MAX-1][0:KV_LORA-1];   // PER-SEQUENCE KV windows
    reg [15:0] KRP [0:NSEQ-1][0:L-1][0:S_MAX-1][0:ROPE-1];
    ...
    integer i,j,e,GLY,sc,SQ;
    reg [SEQW-1:0] g_seq;              // reference DUT: which sequence's KV to serve
    reg [SEQW*B-1:0] seq_w;            // batched DUT per-row seq ids (row0 forced 0)
    task build_stimulus; input integer seed0; input integer band; begin
        ...
        for (GLY=0;GLY<L;GLY=GLY+1) begin
            ...
            for (SQ=0;SQ<NSEQ;SQ=SQ+1) begin   // DISTINCT KV per sequence
                for (i=0;i<S_MAX;i=i+1) for (j=0;j<KV_LORA;j=j+1) begin CKV[SQ][GLY][i][j]=gen_bf16(sc,band); sc=sc+1; end
                for (i=0;i<S_MAX;i=i+1) for (j=0;j<ROPE;j=j+1)    begin KRP[SQ][GLY][i][j]=gen_bf16(sc,band); sc=sc+1; end
            end
    ...
    always @(kc_idx_s or db_layer_s or g_seq or start_s) begin kc_ckv_s={KV_LORA*16{1'b0}}; kc_krope_s={ROPE*16{1'b0}};
        for (cds=0;cds<KV_LORA;cds=cds+1) kc_ckv_s[16*cds+:16]=CKV[g_seq][db_layer_s][kc_idx_s][cds];
        for (cds=0;cds<ROPE;cds=cds+1)    kc_krope_s[16*cds+:16]=KRP[g_seq][db_layer_s][kc_idx_s][cds]; end
    ...
    //========================================================================
    // BATCHED MULTI-SEQ SoC DUT (glm_fp8_soc_ms, PE_M=B, NSEQ=B) -- the top
    //   drives its OWN host FSM (prefill B seqs -> batched forward -> decap);
    //   the TB pulses `start` + prompt tokens + per-seq lengths, then reads
    //   next_tok/done.  The weight-pull responders are IDENTICAL to the model
    //   TB (PE_M-agnostic).  KV DATA is stubbed per (kc_seq, db_layer, kc_idx).
    //========================================================================
    localparam integer KV_CTX_TB   = 1024;
    localparam integer KV_RES_TB    = 16;
    localparam integer ROW_BITS_TB = (KV_LORA + ROPE) * 16;
    localparam integer KVPOSW_TB   = (KV_CTX_TB<=1)?1:$clog2(KV_CTX_TB);

    reg                       start_w;
    wire                      busy_w, done_w, tokv_w;
    reg  [B*TOKW-1:0]         token_id_w;      // prompt tokens (B)
    reg  [(IDXW+1)*B-1:0]     slen_w;          // per-sequence lengths / decode positions
    wire [B*VOCAB*16-1:0]     logits_w;
    wire [B*TOKW-1:0]         argmax_w;        // SoC next_tok (B argmax)
    ...
    glm_fp8_soc_ms #(
        ...
        .THETA(THETA), .PE_N(PE_N), .POSW(POSW), .N_EXPERT(N_EXPERT), .TOPK(TOPK),
        .INTER_MOE(INTER_MOE), .INTER_DENSE(INTER_DENSE), .RSCALE(RSCALE), .TN(TN),
        .BLK(BLK), .LM_TN(LM_TN), .PE_M(B), .N_STEPS(N_STEPS),
        .KV_CTX(KV_CTX_TB), .KV_RESIDENT(KV_RES_TB), .FLASH_LAT(5)
    ) dut_soc (
        .clk(clk), .rst(rst), .start(start_w), .busy(busy_w), .done(done_w),
        .prompt_tok(token_id_w), .s_len_vec(slen_w),
        .next_tok(argmax_w), .tok_valid(tokv_w), .logits(logits_w),
    ...
    //========================================================================
    // per (seq/row, step) decoded tokens: reference (single-seq multi-step) vs batched.
    reg [TOKW-1:0]  ref_loop [0:B-1][0:N_STEPS-1];
    reg [TOKW-1:0]  bat_loop [0:B-1][0:N_STEPS-1];
    integer         prompt_len;                    // set per case

    integer rlk, rlwd, rltk, b, s2, bstep, wd2, fail_this;
    // REFERENCE: run the PE_M=1 model N_STEPS decode steps for sequence `sq`, seeded
    //   by tok0.  Step k queries at position prompt_len+k over the growing extent
    //   prompt_len+k (the CKV responder serves keys 0..extent-1, incl. decode keys
    //   at positions prompt_len.. which are CKV[sq][layer][pos] -- the SAME value the
    //   batched top writes into kv_mem), then feeds its argmax into step k+1.
    task run_ref_loop; input integer sq; input integer tok0; begin
        g_seq = sq[SEQW-1:0];
        rltk  = tok0;
        for (rlk = 0; rlk < N_STEPS; rlk = rlk + 1) begin
            token_id_s = rltk[TOKW-1:0];
            s_len = (prompt_len + rlk);
            pos   = (prompt_len + rlk);
            @(negedge clk); start_s = 1'b1; @(negedge clk); start_s = 1'b0;
            rlwd = 0;
            while (!done_s && rlwd < 400000) begin @(negedge clk); rlwd = rlwd + 1; end
            if (!done_s) begin $display("FAIL: ref sq=%0d step=%0d TIMEOUT", sq, rlk); errors=errors+1; end
            @(negedge clk);
            ref_loop[sq][rlk] = argmax_s;
            rltk = argmax_s;
        end
    end endtask

    // BATCHED: drive the multi-step top ONCE; collect its per-step tok_valid stream.
    task run_batched_loop; begin
        for (s2=0; s2<B; s2=s2+1) slen_w[(IDXW+1)*s2 +: (IDXW+1)] = prompt_len;
        @(negedge clk); start_w = 1'b1; @(negedge clk); start_w = 1'b0;
        bstep = 0; wd2 = 0;
        while (bstep < N_STEPS && wd2 < 1600000) begin
            @(negedge clk); wd2 = wd2 + 1;
            if (tokv_w) begin
                for (b=0; b<B; b=b+1) bat_loop[b][bstep] = argmax_w[TOKW*b +: TOKW];
                bstep = bstep + 1;
            end
        end
        if (bstep != N_STEPS) begin
            $display("FAIL: batched loop collected %0d/%0d steps", bstep, N_STEPS);
            errors=errors+1;
        end
    end endtask

    task do_loop_case; input integer tokA; input integer tokB; begin
        test_count = test_count + 1;
        run_ref_loop(0, tokA);                             // seq 0 decodes N_STEPS tokens alone
        run_ref_loop(1, tokB);                             // seq 1 decodes N_STEPS tokens alone
        token_id_w = { tokB[TOKW-1:0], tokA[TOKW-1:0] };   // batched prompt: row0=A, row1=B
        run_batched_loop;                                  // both decode N_STEPS together
        fail_this = 0;
        for (b=0; b<B; b=b+1)
            for (bstep=0; bstep<N_STEPS; bstep=bstep+1) begin
                if (^bat_loop[b][bstep] === 1'bx) begin
                    $display("FAIL row %0d step %0d: X in batched token", b, bstep); fail_this=1;
                end else if (bat_loop[b][bstep] !== ref_loop[b][bstep]) begin
                    $display("FAIL row %0d step %0d: batched %0d != ref %0d",
                             b, bstep, bat_loop[b][bstep], ref_loop[b][bstep]); fail_this=1;
                end
            end
        if (fail_this) errors = errors + 1;
        else $display("ok  decode-loop {seq0<-%0d, seq1<-%0d}: %0d rows x %0d steps == per-seq multi-step ref  (seq0 stream {%0d,%0d}, seq1 {%0d,%0d})",
                      tokA, tokB, B, N_STEPS,
                      ref_loop[0][0], ref_loop[0][1], ref_loop[1][0], ref_loop[1][1]);
    end endtask
    ...
    initial begin
        ...
        slen_w = {((IDXW+1)*B){1'b0}};
        g_seq = {SEQW{1'b0}}; seq_w = {SEQW*B{1'b0}};
        rst = 1'b1;
        repeat(4) @(negedge clk);
        rst = 1'b0;
        @(negedge clk);

        // MULTI-STEP decode loop (N_STEPS tokens per start).  Two distinct sequences
        //   (row r -> seq r) decode N_STEPS tokens together in ONE batched run; each
        //   row's step-k token must be BIT-EXACT to a standalone PE_M=1 model decoding
        //   that sequence alone for N_STEPS steps (feed argmax back, extent+pos grow per
        //   step).  Decode positions prompt_len .. prompt_len+N_STEPS-1 must be < S_MAX
        //   so the shared CKV serves both the batched top's kv_mem writes and the
        //   reference's kc responder at the SAME decode-position latent.
        // DENSE: prompt_len=1 -> extents 1,2 (<=TOPK) -> the step-0 decode key at
        //   position 1 IS attended in step 1 (dense), exercising the KV feedback path.
        prompt_len = 1; build_stimulus(500,  0); do_loop_case(3, 11);
        // MIXED: prompt_len=2 -> extents 2 (dense), 3 (sparse); positions 2,3.
        prompt_len = 2; build_stimulus(7000, 0); do_loop_case(11, 5);
        // SPARSE: prompt_len=6 -> extents 6,7 (both > TOPK); positions 6,7 < S_MAX=8.
        prompt_len = 6; build_stimulus(90000, 1); do_loop_case(5, 9);
