...
chip or a measured end-to-end token rate. Our central claim is a
...
drawn from two real published GGUF files (F16 is a lossless conversion verified
by the same mixed-type gates), so the silicon inherits the community's
checkpoint ecosystem \emph{by construction}---closing the ``self-referential
golden'' gap endemic to reimplementation-based accelerators. Around this contract we contribute:
(i)~speculative decoding composed into the memory-system top and \emph{proven}
in RTL---the committed token stream is a bit-exact prefix of greedy decoding
(the \code{spec==greedy} invariant), enabled by a new intra-batch causal
attention mode and a die-internal KV write-back, with the bandwidth-amortization
factor $A_{\mathrm{eff}}$ \emph{measured from a hardware weight-load counter}
(an all-accept schedule commits exactly $K{+}1$ tokens per weight load);
(ii)~an adversarial verification discipline in which the load-bearing gates are
paired with fault-injection builds that must fail, exact per-gate test counts
are pinned by the release gate, and---a finding we believe is
underappreciated---%
\emph{output-insensitive} inputs (MoE router codes absorbed by top-$k$
selection; normalization gains) are shown to defeat end-to-end equality checks,
requiring direct per-beat die-input bindings; and (iii)~a memory-bandwidth-bound
residency design methodology whose roofline is parameterized by
\emph{measured} expert-union growth $U(K)$ and speculative accept rates, and
whose stall mechanism is validated on real RTL cycles ($\sim$73$\times$ exposed
stall reduction under residency). The compute datapath places and routes on a
commodity Kintex UltraScale+ FPGA. Throughout, projected token rates are tagged
as estimates; the remaining physical gates---vendor PHY IP and tapeout---are
named, not hidden.
\end{abstract}
...
not in general preserve bitwise numerics. We instead make the contract the headline: the RTL dequantization
...
on every Q4\_K, Q6\_K, and Q8\_0 super-block of two real published GGUF
files---376{,}586{,}240 weights compared as raw \code{uint32}, no tolerance.
By transitivity, the silicon dequantizes the real file exactly as the software
ecosystem does, so it inherits the GGUF checkpoint zoo \emph{by construction}.
...
sparse attention, fine-grained MoE, multi-token prediction) are bit-exact to
independent goldens at the faithful slice of Section~\ref{sec:design}; the
$k$-quant dequantization layer is additionally proven on 376.6M real GGUF
weights. To our knowledge this ecosystem-inheritance contract has not been
...
speculative loop, the intra-batch oracle, the KV write-back, each loopback
family---is paired with a fault-injection build whose failure its make target
asserts; the release gate pins the exact multiset of per-gate test counts (68
gates), catching silently reduced testing; default-off features are held to a
structural or sequential equivalence of the default netlist. We report a finding with, we believe,
...
\item \textbf{Physical realizability evidence.} The compute datapath places and
routes on a commodity Kintex UltraScale+ FPGA (87.5\% LUT, hold met, routed
$F_{\max}$ 46.5\,MHz), the full 753B-shape hierarchy elaborates cleanly, and a
retail-parts residency prototype (1280-bit LPDDR5X, 480\,GB) is supported by an
analytical board routability study (Section~\ref{sec:analysis}).
\end{enumerate}
...
sweep, in which each operator is re-verified at its true 753B-configuration
magnitude under the same check contract as at the slice (the GEMM bit-exact
against the \code{ggml} reference at $K{=}6144$; the router's top-$k$ and
renormalization invariants at 256 experts top-8; SwiGLU, softmax, RMSNorm, and
RoPE against their high-precision goldens at intermediate 2048, length 2048,
dimension 6144, and rotary dimension 64 respectively); (ii)~batched-shape gates in which a
...
and prefetch), an MLA latent-KV pager with storage overflow, weight and boot
loaders, and multi-clock domain-crossing FIFOs. The fabric is
...
Both directions are verified: bit-exact gates with the knob on; and, with the
knob off, a sequential-equivalence proof against the pre-feature commit where
tractable (the weight-ECC path) or a structural cell-histogram equivalence with
a built-in liveness self-test (residency, self-KV, the CDC protocol ports)
elsewhere---so the knobs are optimizations, not risks.
...
longest accepted \emph{prefix of the model's own argmax}---never a raw draft
...
equal a standalone non-speculative reference (31 checks). Two independent fault-injection builds must fail and do: one commits raw
...
runs $K{+}1$ rows over a \emph{shared, already-populated} KV cache; but the
draft at position $p{+}j$ must attend keys at positions $p..p{+}j{-}1$, of
which positions $p{+}1..p{+}j{-}1$ are the \emph{current-token keys of rows
...
index $s{+}i$, flowing through the same RMSNorm, up-projections, RoPE, and DSA
selection as cached keys, with the causal mask arising intrinsically from
per-row key extents. The leaf oracle proves a batched pass over consecutive
positions equals the serial single-row chain, full-logit bit-exact
($K\in\{1,2,3\}$); dropping the causal mask (a row seeing its own key) fails.
...
(documented edit-and-revert procedures in the testbench headers) confirmed the
...
PHY-closure loopback (5 families) & fabric round-trip bit-exact + per-beat bindings & proven \\
Memory/control plane & BMC (7 ctrl + ECC ring) + $k$-induction (5) & proven (formal) \\
Result-invariant knobs, default-off & structural (cell-histogram) or sequential equivalence vs pre-feature commit & proven \\
...
Cycle-accurate stall mechanism & per-token stall on real cycles; residency $73\times$ & measured \\
...
\bottomrule
\end{tabular}
\caption{The verification ledger. Every ``proven'' and ``measured'' row is
reproducible from the repository; the load-bearing composition gates are
paired with fault-injection builds that must fail.}
\label{tab:ledger}
\end{table}
...
built \code{ggml} library. The dequantization is format-level and
file-agnostic---a $k$-quant block's bytes decode identically whatever model
they came from---so bitwise identity on these distributions' blocks transfers
to any $k$-quant file, including the 467\,GB target checkpoint. The FP32 outputs are compared bitwise (as
\code{uint32}, not within a tolerance). All 376{,}586{,}240 weights match. By
transitivity with the RTL$\,\equiv\,$reference gates, the hardware
dequantization equals the real file's \code{ggml} dequantization. The scope is
deliberate: the GEMM's FP32 accumulation order is bit-exact to \emph{our}
documented order, not to llama.cpp's runtime (whose accumulation order
differs); whole-runtime equality is out of contract.
...
the MTP accept rate---first-position $r_1{\approx}0.87$ with steep per-position
decay---which places the memory-bound optimum at shallow depth ($K{=}1$--$2$)
and a production $A_{\mathrm{eff}}{\approx}1.9$ at $K{=}1$; the RTL-measured
...
and routes on a Kintex UltraScale+ XCKU3P (with the wide memory-side ports
terminated in an on-chip harness so the fit is not I/O-limited): 142{,}320 LUTs
(87.5\% utilization), $\sim$100K flip-flops, 421 DSPs, zero BRAM, hold met, and
a routed maximum frequency of 46.5\,MHz reached through
bit-exactness-preserving repipelining rounds (a 4.6$\times$ campaign). This establishes that the
datapath is physically realizable on commodity silicon; it is a compact slice,
not the full model at product speed, and we do not extrapolate a token rate
from it.
...
literature reports roughly 1--13\,\tps\ for GPU/flash offloading of models of
...
from the public repository at \url{https://github.com/Wick-Lim/AIPU} via the
