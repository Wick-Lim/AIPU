...
| **L8** ◐ | repo dead-code quarantine | *(legacy TPU DONE)* the legacy scalar **TPU v2.0** core (16 src + 19 TBs: `tpu_top`/`soc`/`axi`, decoder, regfile, `gemm_systolic`, `conv2d_unit`, `vector_alu`, …) has been **removed** from the repo, off the GLM datapath — **`make all` is GLM-only** (the prove-it gate). Remainder (bf16 golden, redundant `batched_moe`) still in-tree | **0 on the chip** | — | ✔ | low (TPU done) | none (hygiene only) |
