...
> product-deciding variable there), effective band **~76–95 tok/s [EST]**; the ONFI-64ch streaming tier
...
2. **MTP K>1** (P2.2) — extend the speculative loop, keep spec==greedy exact; K=2 sweet spot on the
   ...
   (`spec_depth_adapt` + `spec_decode_seq` `ADAPT`, `make spec-adapt`); GLM-U K-sweep optimum is
   K=4–5 at r=0.9, K=2–3 at r=0.8.)*
