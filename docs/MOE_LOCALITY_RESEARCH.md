...
> 하이브리드 상위 SKU / >512GB 체크포인트에 대해 유효하게 남는다. 수락률 r도 3차
> 측정(잡 B, vLLM MTP 스윕: r₁=0.87 위치별 급감, A_eff ~2.9 고원 → 최적 K=1–2,
> 설계점 ≈80 tok/s [실측-입력 EST])으로 실측 완료 — 남은 것은 GLM-5.2 플래그십
> 자체 확인뿐.
...
> 자체 라우팅은 아직 미확정). 항목 4(수락률 r)는 3차 측정(잡 B, vLLM MTP 스윕)으로
> **완료** — r₁=0.87, 위치별 0.87/0.60/0.32/0.13/0.04, 최적 K=1–2.
