...
| **Raising the context window today would SILENTLY FREEZE attention to the first TOPK tokens** — `DSA_REAL_IDX` is a parameter of `src/mla_attn_q4k.v:170` **and of no other file in the repo** (`grep -rn DSA_REAL_IDX src/` hits that file only). `src/glm_decoder_block_q4k.v:331-337` instantiates the attention passing `PER_ROW_POS/SLEN/SEQ` but **not** `DSA_REAL_IDX`, so the production hierarchy hard-wires it to its default 0, with no top-level override. Per the RTL's own comment (`:155-158`): at 0 the indexer is fed zero key-index vectors, so "every key scores 0 and top-K keeps keys 0..min(S,TOPK)-1 by lower-index tie-break — **Q-INDEPENDENT**". | **Invisible at the committed config and catastrophic the moment it is scaled.** At S_MAX=8/TOPK_ATTN=8, min(S,TOPK)=8 = every key, so selection is a no-op and nothing looks wrong. Raise S_MAX to 1024 and every query at every position attends to **the first 8 tokens of the sequence, forever** — a frozen prefix that ignores the entire conversation. It still runs, still emits fluent-shaped tokens, and still passes any test that does not assert **which** keys were selected. This is why the context window's blocker is **CORRECTNESS, not resources or sim runtime**: the parameter cannot simply be turned up. (DSA_REAL_IDX=0 is a legitimate *default* — it is what makes the pre-DSA datapath byte-identical. The gap is that nothing threads it, so =1 is **unreachable** from the production top.) | **high** | [DESIGN] |
