...
1. **glm_matmul_fp8 per-beat cone** — insert a pipeline register after `fp8_mul`, before the `fixed_to_fp32` + block-scale-dequant + bf16-round back-end. This is the global fmax limiter for both the MoE expert and the dense GEMM path. (The c_out part-select tail is a topo artifact, not real depth — no register needed there.) **✅ DONE (registered the fp8_mul product into a new stage; the real per-beat activation cone 71→62, −12.7%; ACC_DRAIN 3→4; golden 224 bit-exact, argmax 4/31/20 unchanged.)** **✅ ALSO DONE — Ph1 fold-drain pipeline:** the dequant/fold *back-end* was then split too — `fixed_to_fp32` (48-bit leading-one + barrel-shift + RNE) and the bf16→fp32 scale are now registered (`acc_sel_r`/`wf_sel_r`) **before** the block-scale `fp32_mul_pipe`, so the bank-convert no longer shares a register-to-register stage with the mul's combinational 24×24 mantissa multiply. `DEQ_LAT +1` (the side-band valid/slot/a_shift pipe auto-tracks; `out_valid`/busy unchanged → consumers latency-transparent); **BIT-IDENTICAL** (`glm_matmul_fp8_tb` ALL 224, err/tol 0.4317 == baseline; swiglu/mla/model/router all pass). Real **sky130_fd_sc_hd (tt)** timing: the isolated fold segment `accx→fixed_to_fp32→block-scale mul` **15,576→12,390 ps (−20.5%, 64.2→80.7 MHz)**; the full 2×2/K256 module **22,186→14,189 ps (45→70 MHz)** — measured confirmation this back-end fold path, not the MAC, was the real fmax limiter (`docs/PHYSICAL_SKY130.md`). The PE 4×4 raw-abc global path is the `c_out` positional-shifter artifact (below) — untouched by design; ORFS post-placement already resolves it.
