...
module mtp_head_fp8 #(
    ...
    reg [7:0] cemax_r [0:PE_M-1];                 // per-row running exponent-max over cbuf
    // per-row proj activation pow2 shift, packed for the PE_M-wide matmul a_shift.
    wire [8*PE_M-1:0] csh_packed;
    genvar gcs;
    generate
    for (gcs = 0; gcs < PE_M; gcs = gcs + 1) begin : CSH
        assign csh_packed[8*gcs +: 8] = dyn_shift(cemax_r[gcs]);
    end
    endgenerate
    ...
    glm_matmul_fp8 #(.PE_M(PE_M), .PE_N(PROJ_TN), .KMAX(CK), .BLK(BLK)) u_proj (
        .clk(clk), .rst(rst), .start(pp_start), .k_len(pp_klen),
        .in_valid(pp_in_valid), .a_col(pp_a), .w_row(pw_col),
        .a_shift(csh_packed), .w_scale(pw_scale),
            ...
            S_IDLE: begin
                ...
                if (start) begin
                    ...
                    for (rr=0; rr<PE_M; rr=rr+1) begin
                        ...
                        cemax_r[rr] <= 8'd0;    // fresh per-row running exponent-max for cbuf
                    end
            ...
            S_NORM: begin
                ...
                if (cn_y_valid[0]) begin
                    for (rr=0; rr<PE_M; rr=rr+1) begin
                        ...
                        endcase
                        // accumulate the per-row running exponent-max over the concat
                        // cbuf (phases 0/1 only; phase 2 writes xn, not cbuf).
                        if (!cn_phase[1] && (cn_y_out[16*rr + 7 +: 8] > cemax_r[rr]))
                            cemax_r[rr] <= cn_y_out[16*rr + 7 +: 8];
                    end
                end
                if (cn_done[0]) begin
