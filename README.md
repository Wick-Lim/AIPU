...
> §5c — presumes a MAC array sized to consume 1.54 TB/s, i.e. **~12.7K lanes @490MHz** per §3 (at the
> checkpoint's measured 0.62 B/param; ~2x a naive average because attention and MoE run in sequence with
> DEDICATED engines, so each pool must absorb the full bandwidth in its own phase); a smaller
...
- **[`host/`](host/README.md)** — the **host software scaffold**: a local **OpenAI-compatible server** (`python3 host/aipu_server.py`, stdlib only) mirroring the RTL host interface, the **real GLM-5.2 BPE tokenizer** (+ byte fallback), the **GLM chat template**, OpenAI **sampling params**, and 3 backends — `MockDevice`, a **simulator backend** (drives the RTL model slice via `vvp`), and USB (later). `make host-test` (32 tests). The simulator backend targets the on-`main` `glm_model_q4k` slice (`build/glm_model_q4k_full_sim` via `make model-q4k`) and returns the real RTL argmax tokens; its parse/protocol path is covered by `host-test`, the full `vvp` run validated separately. *(Scope: a co-sim datapath witness, not an interactive chatbot — the slice run is minutes-long and replays the TB's fixed golden vectors, not a live prompt.)*
