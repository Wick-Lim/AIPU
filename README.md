...
> §5c — presumes a MAC array sized to consume 1.54 TB/s, i.e. **~13.2K lanes @490MHz** per §3 (at the
> checkpoint's measured 0.62 B/param; 13.2K not 6.6K because attention and MoE run in sequence with
> DEDICATED engines, so each pool must absorb the full bandwidth in its own phase — exactly 2x); a smaller
